  ...
  : public container<N, R>
{
  ...
  template <size_t ... Idx> ALG2_CXX11_CONSTEXPR linear_container(const A& v, Util::index_sequence<Idx...>)
  : container<N, R>{ v[Idx]... }
  {
  }
  ...
  {
    return assign(v, Util::make_index_sequence<N>());
  }
  ...
  ALG2_CXX11_CONSTEXPR A operator -() const
  {
    return negate(Util::make_index_sequence<N>());
  }

  ALG2_CXX11_CONSTEXPR A operator +(const A& v) const
  {
    return add(v, Util::make_index_sequence<N>());
  }
  ALG2_CXX11_CONSTEXPR A operator -(const A& v) const
  {
    return add(-v, Util::make_index_sequence<N>());
  ...
  ALG2_CXX11_CONSTEXPR A operator *(const R t) const
  {
    return multiply(t, Util::make_index_sequence<N>());
  }
  ALG2_CXX11_CONSTEXPR A operator /(const R t) const
  {
    return multiply((R)1/t, Util::make_index_sequence<N>());
  }
  ALG2_CXX11_CONSTEXPR R operator ,(const A& v) const
  {
...
private:
  ...
  template <size_t ... Idx>
  ALG2_CXX14_CONSTEXPR A assign(const A& v, Util::index_sequence<Idx...>)
  {
    return { this->m_data[Idx] = v[Idx] ... };
  }
  template <size_t ... Idx>
  ALG2_CXX11_CONSTEXPR A negate(Util::index_sequence<Idx...>) const
  {
    return { -this->m_data[Idx] ... };
  }
  template <size_t ... Idx>
  ALG2_CXX11_CONSTEXPR A add(const A& v, Util::index_sequence<Idx...>) const
  {
    return { this->m_data[Idx] + v[Idx] ... };
  }
  template <size_t ... Idx>
  ALG2_CXX11_CONSTEXPR A multiply(const R t, Util::index_sequence<Idx...>) const
  {
...
#define LINEAR_CONTAINER
  ...
  namespace Linear
  {
    template <size_t M, size_t N, typename R, class A> struct container : Alg2::container<M*N, R>
    {
      ...
      {
        return assign(m, Util::make_index_sequence<M*N>());
      }
      ...
      ALG2_CXX11_CONSTEXPR A operator -() const
      {
        return negate(Util::make_index_sequence<M*N>());
      }
      ALG2_CXX11_CONSTEXPR A operator +(const A& m) const
      {
        return add(m, Util::make_index_sequence<M*N>());
      }
      ALG2_CXX11_CONSTEXPR A operator -(const A& m) const
      {
        return add(-m, Util::make_index_sequence<M*N>());
      ...
      ALG2_CXX11_CONSTEXPR A operator *(const R t) const
      {
        return multiply(t, Util::make_index_sequence<M*N>());
      }
      ALG2_CXX11_CONSTEXPR A operator /(const R t) const
      {
        return multiply((R)1/t, Util::make_index_sequence<M*N>());
      }
      
    private:
      template <size_t ... Idx>
      ALG2_CXX11_CONSTEXPR A assign(const A& v, Util::index_sequence<Idx...>) const
      {
        return { this->m_data[Idx] = v[Idx] ... };
      }
      template <size_t ... Idx>
    ...
    namespace Trans
    {
      ...
      inline ALG2_CXX11_CONSTEXPR double_variable_map<1, N, R> row(const double_variable_map<M, N, R>& m, size_t idx, Util::index_sequence<Idx...>)
      {
        return { m[idx*M + Idx]... };
      }
      template <size_t M, size_t N, typename R, size_t ... Idx>
      inline ALG2_CXX11_CONSTEXPR double_variable_map<M, 1, R> col(const double_variable_map<M, N, R>& m, size_t idx, Util::index_sequence<Idx...>)
      {
        return { m[idx + Idx*N]... };
      }
      ...
      transpose(const double_variable_map<M, N, R>& m, Util::index_sequence<Idx...>)
      {
    ...
    add(const double_variable_map<M, N, R>& lhs, const double_variable_map<M, N, R>& rhs, Util::index_sequence<Idx...> = Util::make_index_sequence<M*N>())
    {
    ...
    multiply(const double_variable_map<L, M, R>& lhs, const double_variable_map<M, N, R>& rhs, Util::index_sequence<Idx...>)
    {
    ...
    inline ALG2_CXX11_CONSTEXPR
    container<M, N, R, A>
    multiply(const R t, const container<M, N, R, A>& m, Util::index_sequence<Idx...> = Util::make_index_sequence<M*N>())
    {
      return A{ t*m[Idx]... };
    }
    ...
    {
      return multiply(lhs, rhs, Util::make_index_sequence<L*N>());
    }
    template <size_t M, size_t N, typename R, class A>
